...
abstract class LPaperActivity : BaseLPaperActivity<LPaperKonfigs>(), FavsDbManager {
    
    override val prefs: LPaperKonfigs by lazy { LPaperKonfigs(this) }
    
    private val toolbar: CustomToolbar? by bind(R.id.toolbar)
    private val pager: ViewPager? by bind(R.id.pager)
    private var pagerAdapter: LPaperSectionsAdapter? = null
    ...
    private fun initPagerAdapter() {
        pagerAdapter = LPaperSectionsAdapter(
    ...
    private fun scrollToTop() {
        pagerAdapter?.get(lastSection)?.let {
            try {
                (it as? BaseLPaperFragment<*, *>)?.scrollToTop()
    ...
    private fun doSearch(filter: String = "", closed: Boolean = false) {
        pagerAdapter?.get(lastSection)?.let {
            try {
                (it as? BaseLPaperFragment<*, *>)?.enableRefresh(!filter.hasContent())
                synchronized(lock) {
                    postDelayed(150) {
                        (it as? BaseLPaperFragment<*, *>)?.applyFilter(filter, closed)
                    }
                }
            } catch (ignored: Exception) {
    ...
    private fun refreshContent() {
        ...
        adapt?.let {
            pagerAdapter?.get(lastSection)?.let {
                try {
                    (it as? BaseLPaperFragment<*, *>)?.reloadData(
